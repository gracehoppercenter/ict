function init() {
  ...
  if (!hasValidDoctype) {
    console.warn(
      "Warning: This document does not have a <!DOCTYPE html> declaration."
    );
    addWarningFooter();
  }
  ...
  if (isLocalFile) {
    ...
    fetch(
      "https://validator.w3.org/nu/?out=json&doc=" + encodeURIComponent(loc),
      {
          method: "GET"
      })
      .then(response => response.json())
...
<div id="htmlcss">
<p>
<strong>HTML/CSS validation could not be performed due to an error.</strong>
</p>
</div>
`;
}
